...
ggplot()+
  aes(x = temporal$Year, y = temporal$`NIS species` )+
  geom_line() + 
  coord_cartesian(ylim = c(0,60))+
  scale_x_continuous(expand = c(0,0), breaks = seq(1995, 2024, 2)) + 
  scale_y_continuous(expand = c(0,0), breaks = seq(0, 60, 10)) + 
  ylab("Cumulative discoveries") + 
  theme(axis.title.x = element_blank())+
  theme_classic()

model <- snc(y = temporal$`NIS species`, control = list(maxit = 1e4))
model
